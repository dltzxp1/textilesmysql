...
$pe=isset($_REQUEST['pe'])?$_REQUEST['pe']:null;
$script=null;

if(isset($_REQUEST['fa']) && isset($_REQUEST['fb'])){
    if($pe=='TODOS'){
        $script="SELECT * FROM pedido WHERE ped_fecha>='$fa' AND ped_fecha<='$fb'";
    }else{
        $script="SELECT * FROM pedido WHERE ped_fecha>='$fa' AND ped_fecha<='$fb' AND ped_estado='$pe'";
    }
...
$pdf->Cell(20, 8, 'Modelo', 0);
$pdf->Cell(20, 8, 'Trazos', 0);
$pdf->Cell(20, 8, 'Cortador', 0);
$pdf->Cell(20, 8, utf8_decode('D. Gráfico'), 0);
$pdf->Cell(20, 8, 'Despacho', 0); 
$pdf->Cell(20, 8, utf8_decode('Producción'), 0);
$pdf->Cell(20, 8, 'Calidad', 0);
$pdf->Cell(20, 8, 'Empaque', 0);

//$pdf->Cell(20, 8, count($arrProduccion), 0);
...
for($i=0;$i<count($arrPedido);$i++){   
    ...
    $pdf->Cell(20, 8, $arrPedido[$i]->ped_mod_id,0); 
      
    $pdf->Cell(20, 8, $arrPedido[$i]->ped_tra_id,0); 
    $pdf->Cell(20, 8, substr($arrPedido[$i]->ped_cor_id.'..', 0, 8),0);
    $pdf->Cell(20, 8, $arrPedido[$i]->ped_dise_id,0);
    $pdf->Cell(20, 8, substr($arrPedido[$i]->ped_des_id.'..', 0, 8),0);
            
     if(count($arrProduccion)>0){
        for($j=0;$j<count($arrProduccion);$j++){
            if($arrProduccion[$j]->prodc_id==$arrPedido[$i]->ped_prodc_id){
                 $pdf->Cell(20, 8, substr($arrProduccion[$j]->prodc_nombre, 0, 12),0);
            }
        }
    }
    ...
    if(count($arrCalidad)>0){
        for($j=0;$j<count($arrCalidad);$j++){
            if($arrCalidad[$j]->cal_id==$arrPedido[$i]->ped_cal_id){
                 $pdf->Cell(20, 8, substr($arrCalidad[$j]->cal_nombre, 0, 10),0);
            }
        }
    }
    
    $pdf->Cell(20, 8, substr($arrPedido[$i]->ped_emp_id.'..', 0, 8),0);
